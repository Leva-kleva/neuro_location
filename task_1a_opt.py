from conf import *
from helpers import count_score
...
basic_model.add(Dense(units=2, activation='sigmoid', input_shape=(2,)))
...
basic_model.fit(np.array(X_train), np.array(Y_train), epochs=128, validation_data=(X_val, Y_val))


# оценка модели
pred = basic_model.predict(np.array(X_test))
# print(f"SCORE: {roc_auc_score(Y_test, pred)}")
print(f"SCORE: {count_score(Y_test, pred)}")
